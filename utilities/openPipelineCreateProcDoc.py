...
def isValidOpFolder(folder):
    return os.path.exists(folder) and os.path.exists(os.path.join(folder, "openPipeline"))
...
def getAllComments(scriptPath):
    ...
    if (isValidOpFolder(scriptPath)):
        opPath = os.path.join(scriptPath, "openPipeline")
        scriptFiles = [os.path.join(scriptPath, "openPipeline.mel")]
        scriptFiles.extend(getMelFiles(opPath))
        for script in scriptFiles:
            allComments[os.path.basename(script)] = getComments(script)
    else:
        print(f"{scriptPath} is not a valid openPipeline script path.  Please manually set the script path at the bottom of the openPipelineCreateProcDoc.py file.")
    return allComments
...
def htmlWriter(comments, filename):
    ...
    try:
        with open(filename, 'w') as f:
            ...
            f.write(f'<a name="top"><h1>openPipeline for Maya Procedure Documentation ({datetime.date.today().strftime("%b. %d, %Y")})</h1></a>\n')
        ...
        print(f"{filename} could not be opened/created for writing. Please make sure the script path you specified contains an 'openPipeline' folder.")
...
if __name__ == "__main__":
    ...
    htmlFile = os.path.join(scriptPath, "openPipeline", "openPipelineProcedures.html").replace("\\", "/")

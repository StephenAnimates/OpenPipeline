"""
File: openPipeline_loader.py
Description: Main initialization and setup script for openPipeline in Maya.
...
SETUP_WINDOW = "openPipelineInitSetupWindow"
...
SCRIPT_PATH_OPTION_VAR = "openPipeline_scriptPath"
PROJECT_PATH_OPTION_VAR = "openPipeline_projectFilePath"
...
def is_valid_script_path(folder):
    ...
    A valid path contains 'openPipeline_loader.py' and an 'openPipeline' subfolder.
    ...
    py_loader_file = os.path.join(folder, "openPipeline_loader.py")
    sub_folder = os.path.join(folder, "openPipeline")
...
def setup_exec(*args):
    ...
    if cmds.textField(PROJ_PATH_TEXT_FIELD, query=True, editable=True):
        ...
        proj_file_path = os.path.join(script_path, "openPipeline/").replace("\\", "/")
    ...
    if not error:
        ...
        cmds.confirmDialog(
            title="openPipeline - Project Setup Error",
...
def openPipelineSetup():
    """
    Creates and shows the openPipeline Setup UI.
    ...
    cmds.window(SETUP_WINDOW, title="openPipeline Setup", widthHeight=(405, 350), sizeable=False)
    ...
    with cmds.columnLayout(adjustableColumn=True, rowSpacing=5, co=("both", 10)):
        ...
        cmds.text(label='Please specify the folder in which the "openPipeline" folder and loader scripts are located.', align="left")
        ...
        cmds.text(label='By default, the Project File will be located in the "openPipeline" folder.\nYou may set a different location for the Project File here.', align="left")
    ...
    if project_path and project_path != os.path.join(script_path, "openPipeline/").replace("\\", "/"):
...
def openPipeline():
    """
    Main entry point for openPipeline.
    ...
    scripts_folder_name = "openPipeline"
    error = ""
    ...
    if not error:
        print("openPipeline paths are valid. Sourcing modules...")
